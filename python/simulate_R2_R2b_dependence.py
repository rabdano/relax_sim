...
#
R2bs = np.linspace(0.4, 100, 20)
R1 = np.zeros_like(R2bs, dtype="double")
R2 = np.zeros_like(R2bs, dtype="double")
...
for i, R2b in enumerate(R2bs):
    ...
    plt.ylabel(r"M$\rm_z$")
    plt.legend()
    plt.savefig(wd + f"/R1_{R2b:.1f}s-1.png", bbox_inches="tight", dpi=300)
    plt.close()
    ...
    plt.legend()
    plt.savefig(wd + f"/R2_{R2b:.1f}s-1.png", bbox_inches="tight", dpi=300)
    plt.close()

plt.figure(figsize=(16.0 / 2.54, 12.0 / 2.54))
plt.plot(R2bs, R1, marker="o", color="b", ms=5, label=r"R$\rm_1$")
plt.plot(R2bs, R2, marker="o", color="r", ms=5, label=r"R$\rm_2$")
plt.xlabel(r"R$\rm_{2b}$, s$^{\rm-1}$")
plt.ylabel(r"R$\rm_{1,2}$, s$^{\rm-1}$")
plt.legend()
plt.savefig(wd + "/R2_R2b_dependence.png", bbox_inches="tight", dpi=300)
plt.close()
