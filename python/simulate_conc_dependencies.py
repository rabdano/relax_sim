...
#
cs = np.array([1e-9, 0.1e-3, 0.25e-3, 0.5e-3])
R1 = np.zeros_like(cs, dtype="double")
R2 = np.zeros_like(cs, dtype="double")
...
for i, c in enumerate(cs):
    ...
    plt.ylabel(r"M$\rm_z$")
    plt.legend()
    plt.savefig(wd + f"/R1_{c * 1000:f}mM.png", bbox_inches="tight", dpi=300)
    plt.close()
    ...
    plt.legend()
    plt.savefig(wd + f"/R2_{c * 1000:f}mM.png", bbox_inches="tight", dpi=300)
    plt.close()


plt.figure(figsize=(16.0 / 2.54, 12.0 / 2.54))
plt.plot(cs * 1000, R1, marker="o", color="b", ms=5, label=r"R$\rm_1$")
plt.plot(cs * 1000, R2, marker="o", color="r", ms=5, label=r"R$\rm_2$")
plt.xlabel("Concentration, mM")
plt.ylabel(r"R$\rm_{1,2}$, s$^{\rm-1}$")
plt.legend()
plt.savefig(wd + "/concentration_dependence.png", bbox_inches="tight", dpi=300)
plt.close()
